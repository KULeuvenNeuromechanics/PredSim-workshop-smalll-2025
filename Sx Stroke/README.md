...
## Step 1: set-up PredSim
Make sure you have PredSim and associated dependencies installed, see https://github.com/KULeuvenNeuromechanics/PredSim

## Step 2: specify that we are using the 2D model
We want to specify that we are using the 2D model, instead of the 3D model. The 2D model is called gait1018. On line 18 and line 23 of PredSim/main.m, change 'Falisse_et_al_2022' with 'gait1018'. See the documentation of PredSim for further information (https://github.com/KULeuvenNeuromechanics/PredSim).

## Step 3: run a simulation with the 2D model
You can now run a simulation with the 2D model, simply by running the Predsim/main.m script. Once your simulation is done, the results are stored in PredSimResults\gait1018. If this is the first time you ran a simulation, the results are stored in files starting with gait1018_v1

## Step 4: induce weakness of the tibialis anterior
...
Next, replace line 19 of Predsim/main.m (currently empty) with the following line of code:
S = update_settings(S);

## Step 5: run a simulation with a weak TA
...
## Step 6: evaluate the effect of weakness
...
## Step 8: run a simulation with a weak TA and an exoskeleton
  You can now run a simulation with the 2D model, simply by running the Predsim/main.m script. Once your simulation is done, the results are stored in PredSimResults\gait1018. If this is the second time you ran a simulation, the results are stored in files starting with gait1018_v2

## Step 9: evaluate the effect of weakness and exoskeleton assitive
   You can now evaluate the effect of weakness by running the PredSim-workshop-smalll-2025/Sx Stroke/Plotting/compare_devices.m script. 

## Step 10: test different stiffness
